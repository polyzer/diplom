...
def getFeatures(model, adata):
    ...
    for i in range(0,len(adata)):
        ...
        caffe_fc7 = net.blobs['classifier'].data[0].copy()
        sTemp_layer_output = caffe_fc7
        sTemp_layer_output= sTemp_layer_output.reshape(1, 8631)

        sTemp_layer_output = np.sqrt(sTemp_layer_output)
        sFeatures.append(sTemp_layer_output)
    sFeatures = np.array(sFeatures)
    sFeatures = sFeatures.reshape(len(sFeatures), 8631)
    return sFeatures
...
def getIdentificationAccuracy(model, aGallery, aGalleryLabel,aProb, aProbLabel):
    ...
    for i in range(0, sProbFeature.shape[0]):
        ...
        for j in range(0, sGalleryFeature.shape[0]):
            print(sProbFeature[i,:].reshape(1,-1))
            print(sGalleryFeature[j,:].reshape(1,-1))
            sProbFeature_new = np.nan_to_num(sProbFeature[i,:].reshape(1,-1))
            sGalleryFeature_new = np.nan_to_num(sGalleryFeature[j,:].reshape(1,-1))
            temp_similarity = cosine_similarity(sProbFeature_new, sGalleryFeature_new)
...
caffe.set_mode_cpu()
net = caffe.Net("./resnet50_2048_caffe/resnet50_ft.prototxt", "./resnet50_2048_caffe/resnet50_ft.caffemodel",  caffe.TEST)
...
## Gallery Path
sGalPath = 'D:\\kursach_4_kurs\\kursach_4\\model\\3DFace\\gallery'
...
## Probe Path
sProbPath = 'D:\\kursach_4_kurs\\kursach_4\\model\\3DFace\\probeAll'
